#!/bin/ksh
export PS4=' + extrkr_gen_gfs.sh line $LINENO: '
...
then
  ...
  set -x
  err_exit "BAD INPUTS AT LINE $LINENO IN extrkr_gen_gfs.sh"
else
  set +x
  echo " "
  echo " #-----------------------------------------------------------------#"
...
#       pert_num=`    echo "${pert}" | cut -c2-3`           ;
  ...
  *) msg="FATAL ERROR:  Model $cmodel is not recognized."  ;
     ...
     err_exit "UNKNOWN cmodel IN extrkr_gen_gfs.sh";;
...
#J.Peng----2014-10-28---------
  if [ ${cmodel} = "gfs" ]; then
    ...
    PARMlist="(UGRD:850|UGRD:700|UGRD:500|UGRD:200|VGRD:850|VGRD:700|VGRD:500|VGRD:200|UGRD:10 m a|VGRD:10 m a|PRMSL|HGT:925|HGT:850|HGT:700|HGT:500|HGT:300|TMP:500|TMP:250|RH:500)"
    ...
    PARMlist="(UGRD:850|UGRD:700|UGRD:500|UGRD:200|VGRD:850|VGRD:700|VGRD:500|VGRD:200|UGRD:10 m a|VGRD:10 m a|PRMSL|HGT:925|HGT:850|HGT:700|HGT:500|HGT:250|TMP:500|TMP:250|RH:500)"
...
#   ensemble models, the gfs directory is checked for the error-
...
#export COMINgenvit=/ensemble/save/Jiayi.Peng/genesis_vital_${syyyy}
#grep "${old_str}"  ${COMINgenvit}/gen_tc.vitals.gfs.gfso.2012 | \
#      grep -v TEST | awk 'substr($0,6,1) !~ /8/ {print $0}' \
#      >>${TRKDATA}/tmprawvit.${atcfout}.${PDY}${cyc}

if [ ${cmodel} = 'ens'  -o ${cmodel} = 'gfs' ]; then 
grep "${current_str}"  ${COMINgenvit}/all.vitals.gfs.gfso.${syyyy} | \
      grep -v TEST | awk 'substr($0,6,1) !~ /8/ {print $0}' \
	    >>${TRKDATA}/tmprawvit.${atcfout}.${PDY}${cyc}

elif [ ${cmodel} = 'cens'  -o ${cmodel} = 'cmc' ]; then
grep "${current_str}"  ${COMINgenvit}/all.vitals.cmc.cmc.${syyyy} | \
      grep -v TEST | awk 'substr($0,6,1) !~ /8/ {print $0}' \
	    >>${TRKDATA}/tmprawvit.${atcfout}.${PDY}${cyc}

elif [ ${cmodel} = 'fens'  -o ${cmodel} = 'ngps' ]; then
grep "${current_str}"  ${COMINgenvit}/all.vitals.ngps.ngx.${syyyy} | \
grep -v TEST | awk 'substr($0,6,1) !~ /8/ {print $0}' \
		  >>${TRKDATA}/tmprawvit.${atcfout}.${PDY}${cyc}

elif [ ${cmodel} = 'eens'  -o ${cmodel} = 'ecmwf' ]; then
grep "${current_str}"  ${COMINgenvit}/all.vitals.ecmwf.emx.${syyyy} | \
      grep -v TEST | awk 'substr($0,6,1) !~ /8/ {print $0}' \
		  >>${TRKDATA}/tmprawvit.${atcfout}.${PDY}${cyc}
...
#
#ttrkdir=/ensemble/save/wx20tm/trak/prod/data
#ttrkdir=/ensemble/save/wx20tm/trak/para/scripts
...
# with 4-digit years, however it is unknown when other ensemble
...
then
  ...
  then
    ...
    echo " "
    set -x
    err_exit "ERROR RUNNING supvit_g2 IN extrkr_gen_gfs LINE $LINENO"
  fi
...
#J.Peng----2012-05-04---do not need calculate this ----------
num_gen_vits=0
...
then
  ...
  then
    ...
    echo " "
    set -x
    err_exit "ERROR RUNNING SUPVIT_GEN IN extrkr_gen_gfs.sh LINE $LINENO"
  fi
    ...
    do
      ...
      if [ $attempts -gt 40 ] && [ ! -s $fensfile ]; then
        if [ "$DCOM_STATUS" = "data of opportunity" ]; then
          ...
          err_exit "$fensfile still not available after waiting 40 minutes"
        fi
      fi
...
#J.Peng----2014-10-15---added for U-shear-----
      ...
      if [ $rcc -ne 0 ]; then
        ...
        echo " "
        set -x
        err_exit "ushear_g2.x- ERROR AT extrkr_gen_gfs.sh LINE $LINENO"
      fi
  ...
  then
    ...
    do
      ...
      then
        ...
        echo " "
        set -x
        err_exit "MISSING GFS FILE IN extrkr_gen_gfs.sh: ${gfsdir}/${gfsgfile}${fhour}.grib2"
      fi
  ...
  then
    ...
    do
      ...
      then
        set +x
        echo " "
        echo "FATAL ERROR:  GEFS ${PERT} File missing: ${ensdira}/${ensgfilea}${fhour}"
        echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
        echo " "
        set -x
        err_exit "MISSING GEFS FILE IN TRACKER SCRIPT: ${ensdira}/${ensgfilea}${fhour}"
      fi
...
#   --------------------------------------------
    ...
    do
      ...
      set +x
      echo " "
      echo "Date in T-mean/U-shear  for pert= $pert and fhour= $fhour before = `date`"
      echo " "
      set -x
...
# ------------------------------
#   Process CMC deterministic , if selected
# ------------------------------
...
then
  ...
  then
    ...
    if [ -s ${TRKDATA}/cce${pert}gribfile.${PDY}${cyc} ]
    then
    ...
    do
      ...
      if [ ! -s ${ccedir}/${ccegfile}${fhour}${ccegfile2} ]
      then
        set +x
        echo " "
        ...
        echo " "
        set -x
        err_exit "MISSING CMC Ens FILE IN TRACKER SCRIPT: ${ccegfile}${fhour}${ccegfile2}"
      fi
...
#J.Peng----2014-10-22----added for U-shear-----
	ffile=${gfile}
	ifile=${TRKDATA}/cce${pert}ixfile.${PDY}${cyc}
...
# Input files
namelist_2_fix=${TRKDATA}/namelist_2_fix_input.${PDY}${cyc}
echo "&timein maxhrs=${fcstlen},"       >${namelist_2_fix}
echo "        dthrs=${vit_incr}/"       >>${namelist_2_fix}

# Output file
export FORT10=${TRKDATA}/${cmodel}.genesis_leadtimes_120

${EXECens_tracker}/leadtime <${namelist_2_fix}
rcc=$?
...
if [ $rcc -ne 0 ]; then
  ...
  echo " "
  set -x
  err_exit "leadtime - ERROR AT extrkr_gen_gfs.sh LINE $LINENO"
fi
...
if [ ${model} -eq 1 ]; then
  export FORT15=${TRKDATA}/${cmodel}.genesis_leadtimes_120
else
...
#    cp ${TRKDATA}/trak.${atcfout}.atcf_gen.${regtype}.${PDY}${cyc} ${COMOUT}/
  ...
  echo " "
  set -x
  err_exit "ERROR RUNNING gettrk_gen_gfs IN 2nd step"
fi
