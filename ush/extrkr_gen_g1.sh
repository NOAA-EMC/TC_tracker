#!/bin/ksh
export PS4=' + extrkr_gen_g1.sh line $LINENO: '
...
elif [ ${cmodel} = 'ukes'  -o ${cmodel} = 'ukmet' ]; then
grep "${current_str}"  ${COMINgenvit}/all.vitals.ukmet.ukx.${syyyy} | \
      grep -v TEST | awk 'substr($0,6,1) !~ /8/ {print $0}' \
                        >>${DATA}/tmprawvit.${atcfout}.${PDY}${cyc}
...
# a ensemble grid with 12-hourly resolution out to 240h.  Previously, we
...
#J.Peng----2012-06-06------------------------------
    gcut=${DATA}/ece.orig.cut.${PDY}${cyc}
    xcut=${DATA}/ece.orig.cut.${PDY}${cyc}.i
    if [ -s ${gcut} ]; then rm ${gcut}; fi
    if [ -s ${xcut} ]; then rm ${xcut}; fi
...
#    for var in u10 v10 u500 v500 u700 v700 u850 v850 z200 z500 z700 z850 z925 t250 t500 mslp
    for var in u10m v10m u850 v850 u700 v700 u500 v500 u200 v200 z1000 z850 z700 z500 z200 t500 t200 mslp rh500
    do
...
#      ecegfile=ecmwf_enspost.${PDY}${cyc}.${var}
    ecegfile=ensposte.t${cyc}z.${var}hr
      ...
      ${WGRIB:?} -PDS10 ${ece_orig} | grep "${pert_grep_str}" | \
            awk -F:TR= '{print $1}'                       | \
            ${WGRIB:?} -i ${ece_orig} -grib -append -o ${gcut}
      export err=$?; err_chk
    
    done
    
    mv ${gcut} ${DATA}/ece${pert}gribfile.${PDY}${cyc}

    # Now go through and do the interpolation and averaging for the various 
    # cyclone phase parameters....
