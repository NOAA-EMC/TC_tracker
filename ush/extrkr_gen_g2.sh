#!/bin/ksh
export PS4=' + extrkr_gen_g2.sh line $LINENO: '
...
then
  ...
  set -x
  err_exit "BAD INPUTS AT LINE $LINENO IN extrkr_gen_g2.sh"
else
  set +x
  echo " "
  echo " #-----------------------------------------------------------------#"
...
case ${cmodel} in 
  ...
  gfs) set +x; echo " "                                    ;
       ...
       echo " "; set -x                                    ;
#       gfsdir=${gfsdir:-${COMROOTp1}/gfs/prod/gfs.${PDY}}  ;
...
# jpdtn=0 for deterministic data.
  ...
  ngps) set +x; echo " "                                   ;
       ...
       echo " "; set -x                                    ;
#       ngpsdir=${ngpsdir:-${COMROOTp1}/fnmoc/prod/navgem.${PDY}};
...
#                 060 066 072 084 096 108 120'              ;
  ...
  cmc) set +x; echo " "                                   ;
       ...
       echo " "; set -x                                    ;
       cmcdir=${cmcdir:-${DCOM:?}}                         ;
#       cmcdir=/gpfs/hps/emc/ensemble/noscrub/Jiayi.Peng/cmc_25km_data   ;
...
# jpdtn=0 for deterministic data.
  ...
  fens) set +x; echo " "                                   ;
       ...
       pert=` echo ${pert} | tr '[A-Z]' '[a-z]'`           ;
       PERT=` echo ${pert} | tr '[a-z]' '[A-Z]'`           ;
#       fensdir=${fensdir:-${DCOMROOT}/us007003/${PDY}/wgrbbul/fnmocens_gb2};
...
#       ccedir=/dcom/us007003/${PDY}/wgrbbul/cmcens_gb2     ;
#       ccegfile=${PDY}${cyc}_CMC_naefs_latlon1p0x1p0_P     ;
#       pert_num=`    echo "${pert}" | cut -c2-3`           ;
#       dcegfile=_0${pert_num}.grib2                        ;
...
#       ccedir=${ccedir:-/dcom/us007003/${PDY}/wgrbbul/cmcens_gb2}   ;
       pert_num=`    echo "${pert}" | cut -c2-3`           ;
...
# jpdtn=0 for deterministic data.
  ...
  *) msg="FATAL ERROR:  Model $cmodel is not recognized."  ;
     ...
     err_exit "UNKNOWN cmodel IN extrkr_gen_g2.sh";;
...
then
  ...
  then
    ...
    echo " "
    set -x
    err_exit "ERROR RUNNING SUPVIT_GEN IN extrkr_gen_g2.sh LINE $LINENO"
  fi
  ...
  then
    ...
    do
      ...
      then
        ...
        echo " "
        set -x
        err_exit "MISSING GFS FILE IN extrkr_gen_g2.sh: ${gfsdir}/${gfsgfile}${fhour}"
      fi
...
then
  ...
  then
    ...
    do
      ...
      if [ ! -s ${ensdira}/${ensgfile}${fhour} ]
      then
        set +x
        echo " "
        echo "FATAL ERROR:  GEFS ${PERT} File missing: ${ensdira}/${ensgfile}${fhour}"
        echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
        echo " "
        set -x
        err_exit "MISSING GEFS FILE IN TRACKER SCRIPT: ${ensdira}/${ensgfile}${fhour}"
      fi
  
#      gfile=${ensdira}/${ensgfile}${fhour}
      gfile=${TRKDATA}/${ensgfile}${fhour}
      cat ${ensdira}/${ensgfile}${fhour} ${ensdirb}/${ensgfileb}${fhour} > $gfile
...
#        echo "FATAL ERROR:  CANADIAN ENSEMBLE ${PERT} missing: ${ccegfile}${fhour}"
        ...
        echo " "
        set -x
#        err_exit "MISSING CMC Ens FILE IN TRACKER SCRIPT: ${ccegfile}${fhour}"
