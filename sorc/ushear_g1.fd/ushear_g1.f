      program ushear
c
c     ABSTRACT: This program calculate the U-shear from an input 
...
c     fort.51----output U-shear grib file at 525hPa
c     Written by Jiayi Peng ---2012-04-18------
...
c
  ...
  201 continue
      ...
      print *,'*---------------------------------------------*'
      print *,' '
      print *,' +++ Top of U-shear +++ '
      print *,' '
      print *,'After Ushear namelist read, input forecast hour= '
     &       ,ifcsthour
...
c      endif
      ...
      if (iogret /= 0) then
        print '(/,a45,i4,/)','!ERROR: in Ushear open_grib_files, rc= '
     &        ,iogret
        STOP 93
      endif
      ...
      if (iha /= 0 .or. iho /= 0 .or. iva /= 0 .or. irfa /= 0) then
        print *,' '
        print *,'!!! ERROR in Ushear allocating arrays.'
     ...
     &             ,readflag,xinptmp,ifcsthour,iparm,igdret)
      ...
      call shear_data (kf,valid_pt,nlevsin,ilevs,readflag
...
c
      ...
      jpds(5)  = iparm  ! Get a U-speed record
     ...
     &                     kf,k,kpds,kgds,lb,f,iret)
      ...
      if (iret.ne.0) then
        print *,' '
        print *,'!!! ERROR in Ushear getgridinfo calling getgb'
...
c
      ...
      level_loop: do lev = 1,nlevsin
        ...
        print *,' '
        print *,'In Ushear getdata read loop, lev= ',lev,' level= '
     &         ,ilevs(lev)
        ...
        jpds(5) = iparm       ! parameter id for U-speed
     ...
     &                        kf,k,kpds,kgds,lb,f,iret)
        ...
        print *,' '
        print *,'After Ushear getgb call, j= ',j,' k= ',k,' level= '
     &         ,ilevs(lev),' iret= ',iret
  ...
  31      format (' rec#  parm# levt lev  byy   bmm  bdd  bhh  fhr   np'
     ...
     &         k,(kpds(i),i=5,11),kpds(14),kf,dmin,dmax
          ...
          print *,' '
          print *,'!!! ERROR: Ushear READ FAILED FOR LEVEL LEV= ',LEV
          print *,' '
...
c
c-----------------------------------------------------------------------
      subroutine shear_data (kf,valid_pt,nlevsin,ilevs,readflag
     &                 ,xinptmp,xouttmp,iidret)
c
c     ABSTRACT: This routine minus U-data between 200 and 850 mb to get
c     an U-shear at 501 mb.   
...
c
      print *,'*----------------------------------------------*'
      print *,'         Calculate U-shear data routine'
      print *,'*----------------------------------------------*'
      print *,' '
...
c      kgds(19) =       0  ;    kgds(20) =     255
      ...
      do lev = 1,nlevsout 
        ...
        print *,'ushear:  just before call to putgb, kf= ',kf
        call putgb (lout,kf,kpds,kgds,valid_pt,xouttmp,ipret)
        print *,'ushear:  just after call to putgb, kf= ',kf
        if (ipret == 0) then
          print *,' '
...
cPeng 05/28/2018 Bug Fixed for FV3-GFS Job Crashed
      ...
      print *,' '
      print *,'Ushear baopen: igoret= ',igoret,' iioret= ',iioret
     &       ,' iooret= ',iooret
