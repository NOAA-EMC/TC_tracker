...
cJ.Peng----2012-04-18----------
      ...
      real      xsfclon,xsfclat,cc_time_pct
...
c       If not enough tracked parms were read in, exit the program....
     ...
     &                   ,trkrinfo,-99.0,cps_vals,wcore_flag,ioaxret)
cJ.Peng----2012-04-18--------
...
c           to get a guess position for the next forecast hour.
 ...
 432                  format (1x,'!!! Fcst hr  = ',i4,':',i2.2)
     ...
     &                    ,-999.0,inp,ist
     &                    ,ifcsthour,0.0
     &                    ,0.0,vradius,maxstorm,trkrinfo,-99.0
...
c           knots (1.9427) is explained in output_atcf.
 ...
 442          format (1x,'forecast hour= ',i4,':',i2.2,' follow:')
     ...
     &                    ,trkrinfo,rmax,cps_vals,wcore_flag,ioaxret)
...
cJ.Peng----2012-04-18------U-shear and RH max and mean values-------
                imeanushe = -999.0
                igridushe = -999.0
                imeanrhum = -99.0
                igridrhum = -99.0
...
c         
        print *,' '
cJP07122013        write (6,'(1x,a17,f6.4,5x,a7,f6.4)') ' R2(r_squared) = ',R2
c     &       ,'   r = ',sqrt(R2)
        write (6,*) ' R2(r_squared) = ',R2
     &       ,'   r = ',sqrt(R2)
        print *,' '
        print *,' *--------------------------------------------------* '
      ...
      if (ie > imax) then
        if (trkrinfo%gridtype == 'global') then 
          ...
          if ( verb .ge. 1 ) then
            ...
            print *,'!!! Returning to calling '
            print *,'!!! routine with a non-zero return code.'
...
c
      ...
      integer intlon,intlat,irmax,output_fhr,ic
...
c       later on in subroutine  wtavrg.
        ...
        if (stderr_close > 0.0) then
          ...
          do ip=1,maxtp
            if (calcparm(ip,ist)) then
              ...
              if ( verb .ge. 3 ) then
                write (6,*) ip,kprm,dist_from_mean(ip),devia(kprm)
     ...
     &               ,360.-temp_clon(kprm),temp_clat(kprm)
          ...
          enddo
c  113     format (1x,'ip= ',i2,' kprm= ',i2,' dist_from_mean= ',f7.3
c     &           ,' devia= ',f7.3,' wtpos= ',f7.5,2x,3(2x,f7.2))
        else
c     
...
c     These (i,j) coordinates are returned as ilonfix and jlatfix.
 ...
 521        format (1x,' for zeta values will not be done.')
        ...
        if ( verb .ge. 3 ) then
          write (6,*) n,ilev,xmeanzeta,imeanzeta(n)
c 621      format (1x,'+++ RPT_MEAN_ZETA: n= ',i2,' lev= ',i4
c     &         ,' xmeanzeta= ',f9.7,' imeanzeta= ',i4)
     ...
     &     ,trkrinfo,ifilret)
        ...
        if ( verb .ge. 3 ) then
          write (6,*) n,ilev,gridpoint_maxmin,igridzeta(n),ifilret
c 623      format (1x,'+++ RPT_GRID_ZETA: n= ',i2,' lev= ',i4
c     &         ,' grid zeta= ',f9.7,' igrid zeta= ',i4,' ifilret= ',i3)
...
cJ.Peng---2012-06-06-ECMWF data parameter has been changed-----
...
c                (21) UKMET hi-res (from NHC)
     ...
     &    inp%model == 22 .or. inp%model == 15) then
        ...
        iglev(10)    = 10
        iglev(11)    = 10
c      else if (inp%model == 15) then   ! CMC: "Eta level 1.000" winds
c        iglevtyp(10) = 119
c        iglevtyp(11) = 119
c        iglev(10)    = 10000
c        iglev(11)    = 10000
...
c                                        pmsl level ID
     ...
     &                        kf,k,kpds,kgds,lb,f,iret)
        ...
        if ( verb .ge. 3 ) then
          print *,' '
          if (inp%lt_units == 'minutes') then
            print *,'After getgb call, j= ',j,' k= ',k,' iftotalmins= '
...
c        read (lgvcard,24,END=801,ERR=891) tmpstorm(ii)
        read (lgvcard,24,END=801) tmpstorm(ii)
        ii = ii + 1
      enddo
...
c
      ...
      logical(1), allocatable :: lb(:)
      integer, parameter :: jf=40000000
      real      xhold,xlondiff,xlatdiff,temp
      real, allocatable :: f(:)
      ...
      integer   iscanflag,iggret,kf,k,lugb,lugi
...
c
 ...
 83     format (' Max Lat: ',f8.3,'  Max Lon: ',f8.3)
        ...
        print *,'extend all the way to the gds-defined grid boundary,'
        print *,'due to the fact that data have been interpolated from'
        print *,'a NPS or Lamb-Conf grid onto a lat/lon grid.  This'
        print *,'program checks the logical bitmap for valid data '
        print *,'points, but just keep this in mind if trying to debug'
        print *,'errors that occur near the grid boundaries for '
        print *,'regional models.'
      endif
...
c
c---------------------------------------------------------------------
cJ.Peng----2012-04-18------10X10 area mean temperature-----------------
c-----------------------------------------------------------------------
